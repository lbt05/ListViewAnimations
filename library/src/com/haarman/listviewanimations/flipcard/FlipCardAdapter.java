...
public class FlipCardAdapter extends BaseAdapterDecorator implements
	...
	public FlipCardAdapter(BaseAdapter baseAdapter, int frontViewResourceId,
			int backViewResourceId) {
		this(baseAdapter, frontViewResourceId, backViewResourceId, null);
	}

	public FlipCardAdapter(BaseAdapter baseAdapter, int frontViewResourceId,
			int backViewResourceId, PreFlipAction preAction) {
		super(baseAdapter);
	...
	@Override
	public void onViewSwiped(View flipView, int flipPosition,
			boolean flipFromRight) {
		// TODO Callback API
	...
	public void setAbsListView(AbsListView listView) {
		mFlipCardListViewTouchListener = new FlipCardListViewTouchListener(
				listView, this);
		...
		listView.setOnScrollListener(mFlipCardListViewTouchListener
				.makeScrollListener());
	}
	...
	public void applyRotation(View v, boolean turnBack, boolean flipFromRight) {
		...
		final Rotate3dAnimation rotation = new Rotate3dAnimation(wise[0],
				wise[1], centerX, centerY, ROTATION_DEPTH, true);
		rotation.setDuration(ROTATION_DURATION);
		rotation.setFillAfter(false);
		rotation.setInterpolator(new AccelerateInterpolator());
		rotation.setAnimationListener(new DisplayNextView(turnBack, v,
				flipFromRight));
	...
	private final class DisplayNextView implements AnimationListener {
		...
		private DisplayNextView(boolean turnBack, View parent,
				boolean flipFromRight) {
	...
	private final class SwapViews implements Runnable {
		...
		public void run() {
			...
			rotation = new Rotate3dAnimation(wise[0], wise[1], centerX,
					centerY, ROTATION_DEPTH, false);
			rotation.setDuration(ROTATION_DURATION);
